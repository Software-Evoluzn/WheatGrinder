  ...
  return (
    <>
      ...
      <AppDialog
        ...
        icon="help-circle"
        title="Help"
        message="The hopper is empty. Load more grain, or go back if you'd rather stop here."
        confirmLabel="Got it"
...
/* -------------------------------------------------------------------------- */
/*  GrainOverHero — warning-toned status visualization (attention pulse)       */
/* -------------------------------------------------------------------------- */
const HERO = 260;
  ...
  return (
    <View style={styles.hero}>
      ...
      <View style={styles.disc}>
        <Feather name="inbox" size={46} color={colors.warning} />
      </View>
    </View>
  );
};
...
const GrainOverScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      navigation.navigate('LoadGrainsToStart');
    }, 5000);
  ...
  */
  ...
  return (
    <Screen background={colors.background}>
      {/* Back button was added on request (a deliberate navigation change). */}
      <MainHeader
      ...
      <Pressable style={{ flex: 1 }} onPress={handleNavigateNext} accessibilityRole="button">
        <View style={styles.content}>
          <GrainOverHero />

          <Text style={styles.title}>Grain over</Text>
          <Text style={styles.message}>
            The hopper is empty. Load more grain or switch off the machine to finish.
          </Text>

          <WarningPill label="Refill needed" />

          <FootNote style={{ marginTop: spacing.xxl }}>Tap to load grain</FootNote>
        </View>
      </Pressable>
    </Screen>
  );
...
const styles = StyleSheet.create({
  ...
  statusText: { fontSize: 13, fontWeight: '800', color: colors.warning, letterSpacing: 0.5 },
});